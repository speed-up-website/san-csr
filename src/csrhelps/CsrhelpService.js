(function() {
    ...
    function CsrhelpService($q) {
        ...
        function make_any(certf, context) {
			...
			var string_val = certf.hostname;
			
			//remove all whiteSpace from the string
			var string_filtered = string_val.replace(/\s/g, "");
			var domains_array = string_filtered.split(",");			
						
			var length_previous = domains_array.length;
			var i;			
			
			for (i = 0; i < length_previous; i++) {				
				if(certf.www == true){
				//wildcard domains don't need to add 'www.'
				if(domains_array[i].search(/\*/) === -1){
					domains_array.push("www." + domains_array[i]);
				}
				}
				
				//include the base domain too of a wildcard domain, if not exist in the domains_array - in every case whether or not www option is selected
				if(domains_array[i].search(/\*/) !== -1){
					var base_domain = "";
					base_domain = domains_array[i].replace("*.", "");
					domains_array.push(base_domain);
				}
			}	
			...
			*/
            ...
            sequence = sequence.then(function(result) {
				...
				for (i = 0; i < domains_array.length; i++) {
					domains_all.push(new org.pkijs.simpl.GENERAL_NAME({
										   NameType: 2,
										   Name: domains_array[i]
										   }));
				}
...
//Reset the link and text area upon domain text value change and/or select option value change
            ...
            sequence.then(function(result) {
				...
				*/
				...
				//Scroll down to the result div - problematic
				/*
				$('html, body').animate({
					scrollTop: $("#resultproxy").offset().top
				}, 500);*/
