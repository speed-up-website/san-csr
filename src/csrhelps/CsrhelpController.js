  ...
  function CsrhelpController(
    ...
    $log, $q, $scope, $timeout) {
        ...
        var RSAKeySizes = [{key:2048, value:"2048"}, {key:3072, value:"3072"}, {key:4096, value:"4096"}];
		...
		$scope.algorithms = ["RSA"];
        $scope.keysizes = [{key:2048, value:"2048"}, {key:3072, value:"3072"}, {key:4096, value:"4096"}];
		//$scope.wwwoptions = [{key:'--', value:"Add WWW with every domain?"}, {key:'yes', value:"Yes"}, {key:'no', value:"No"}];
        ...
        $scope.copyToClipboard = function($event, item) {
            execCommandCopy($event, item, function(err){
                ...
                for(var i = 0; i < 7; i++){
                    $scope.messages[i].status="Copied";
                }
                item.status="Copied";
            });             
        }
        ...
        $scope.download = function($event, item) {
			...
			var string_filtered = $scope.certificate.hostname.replace(" ", "");
			var domains_array = string_filtered.split(",");
			
			var file_name = domains_array[0].replace("*", "star")
			/*
            if($scope.certificate.hostname=="*.example.com"){
                $scope.certificate.filehostname = "star.example.com";
            }
            else{
                $scope.certificate.filehostname = file_name;
            }
			*/
            $($event.currentTarget)
                .attr('href','data:application/pkcs8,'+encodeURIComponent($scope.messages[0].privateKey))
                .attr('download', file_name+'.key');
